...
export async function verifyKeys(req: AuthenticatedRequest, res: Response) {
  ...
  if (gemini_key && gemini_key.trim().length > 5) {
    try {
      const resp = await axios.get(
        `https://generativelanguage.googleapis.com/v1beta/models?key=${gemini_key.trim()}`,
        { timeout: 8000 }
      );
      if (resp.status === 200) {
...
export async function saveKey(req: AuthenticatedRequest, res: Response) {
  if (!req.user) {
...
export async function getKeysStatus(req: AuthenticatedRequest, res: Response) {
  if (!req.user) {
    return res.status(401).json({ success: false, message: 'Unauthorized' });
  }
